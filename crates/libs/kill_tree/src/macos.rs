use crate::core::{Config, Error, ProcessId, ProcessIds, ProcessInfo, ProcessInfos, Result};
...
pub(crate) mod blocking {
    ...
    pub(crate) fn get_process_info(process_id: ProcessId) -> Result<ProcessInfo> {
        ...
        let name =
            unsafe { std::ffi::CStr::from_ptr(std::ptr::addr_of!(proc_bsdinfo.pbi_name[0])) }
                .to_string_lossy()
                .to_string();
        Ok(ProcessInfo {
            process_id,
    ...
    pub(crate) fn get_process_ids() -> Result<ProcessIds> {
        let buffer_size_sign = unsafe {
            libproc::proc_listpids(libproc::PROC_ALL_PIDS, 0_u32, std::ptr::null_mut(), 0)
        };
    ...
    pub(crate) fn get_process_infos() -> Result<ProcessInfos> {
        let process_ids = crate::macos::blocking::get_process_ids()?;
        let mut process_infos = ProcessInfos::new();
        for process_id in process_ids {
            let process_info = match get_process_info(process_id) {
                Ok(x) => x,
                Err(e) => {
...
pub(crate) mod tokio {
    ...
    #[instrument]
    async fn get_process_info(process_id: ProcessId) -> Result<ProcessInfo> {
        ::tokio::task::spawn_blocking(move || crate::macos::blocking::get_process_info(process_id))
            .await?
    }

    #[instrument]
    async fn get_process_ids() -> Result<ProcessIds> {
        ::tokio::task::spawn_blocking(|| crate::macos::blocking::get_process_ids()).await?
    }

    #[instrument]
    pub(crate) async fn get_process_infos() -> Result<ProcessInfos> {
        let process_ids = crate::macos::tokio::get_process_ids().await?;
        let mut tasks: ::tokio::task::JoinSet<Result<ProcessInfo>> = ::tokio::task::JoinSet::new();
        for process_id in process_ids {
            tasks.spawn(crate::macos::tokio::get_process_info(process_id));
        }
        let mut process_infos = ProcessInfos::new();
